...
`Sallyn` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/Sallyn/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/Sallyn/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee Sallyn` to
...
python ./.trellis/scripts/task.py archive 00-join-sallyn
```
